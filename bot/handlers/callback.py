...
from bot.keyboards import paginated_results_keyboard
...
async def download_cb(client, callback_query: CallbackQuery):
    ...
    status = await callback_query.message.reply_text("⏳ Starting download…")
...
async def paginate_cb(client, callback_query: CallbackQuery):
    ...
    await callback_query.edit_message_reply_markup(
        reply_markup=paginated_results_keyboard(list_token, list_entry["entries"], page)
    )
