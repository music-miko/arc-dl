...
from bot.database import all_user_ids, remove_user, user_count
...
async def broadcast_cmd(client, message: Message):
    if not message.reply_to_message:
        await message.reply_text("↩️ Reply to the message you want to broadcast with /broadcast.")
        return
    ...
    for i, uid in enumerate(user_ids, start=1):
        ...
        if i % 25 == 0:
            try:
                await status.edit_text(f"📣 Broadcasting… `{i}/{total}` (✅ {sent} ❌ {failed})")
            except Exception:
                pass
