...
from .. import LOGGER
from ..utils.common import duration_to_seconds, guess_kind_from_ext, sanitize_filename
from .ffmpeg import ensure_audio, probe_video_meta


class MediaSniffer:
    """Identifies a file's real kind (photo/video/...) from its magic
    bytes or extension when the CDN/HTTP response didn't say clearly.
    The only consumer of this is MediaDownloader below, so it lives here
    rather than in its own single-purpose utils module."""
    ...
    async def _download_http_once(
        ...
        async with aiohttp.ClientSession(headers=self.cdn_headers) as session:
            async with session.get(url, timeout=timeout) as r:
                ...
                # Prefer the real filename when the caller already knows it
                # (e.g. Terabox hands us the original "*.mkv" name) — this
                # is far more reliable than guessing from a CDN URL that
                # may carry no extension at all (or point at a stream
                # manifest rather than the actual file).
