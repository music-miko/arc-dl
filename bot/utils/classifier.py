...
class MessageClassifier:
    def __init__(self):
        ...
        # Terabox is handled separately from the other social kinds: it
        # can resolve to multiple files and is restricted to DM only (see
        # handlers/search.py + handlers/inline.py), so it never joins
        # social_patterns/social_kinds below.
        self.terabox_re = re.compile(
            r"^https?://(www\.)?"
            r"(1024terabox|terabox|teraboxapp|teraboxshare|teraboxlink|"
            r"nephobox|4funbox|mirrobox|momerybox|freeterabox|1024tera|teraboxdl)"
            r"\.(com|app|net)/.+",
            re.I,
        )

        # Apple Music / JioSaavn: a link can point at a single track or at
        # an album/playlist containing many, so — like Spotify — they get
        # their own "_track" vs "_playlist" kinds instead of joining the
        # single-cdn social_patterns list.
    ...
    def classify(self, text: str) -> tuple[str, str]:
        ...
        if apple_match:
            ...
            # A track link ("song", or an "album" link carrying ?i=<id>)
            # downloads directly; a bare album/playlist link lists tracks.
